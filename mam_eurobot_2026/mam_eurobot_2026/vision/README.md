...
- **Run(it's not registered as a node)**:
...
- **Outputs**: `/camera/features/cursor_center` (`geometry_msgs/PointStamped`) for the detected blob centroid and an OpenCV window showing the mask.
- **Runtime parameters**: override HSV bounds or morphology via `--ros-args -p h_min:=... -p h_max:=... -p erode:=...`.
- **Run(example)**:
  ```bash
  ros2 run mam_eurobot_2026 color_detector.py --image-topic /top_camera/image_3
  ```
...
- **Inputs**: subscribes to every `sensor_msgs/Image` topic listed under `cameras` in `vision_settings.yaml` (currently only `/top_camera/image_3` for `observation_device`). Each camera entry also specifies marker IDs/size and world pose hints.
- **Outputs**:
